# How the jktlug Website Works

This document explains the architecture of the jktlug website — what
...
When a browser visits `https://jktlug.jp/` (or `localhost:8000` locally),
...
- `<head>` with charset, title, and CSS link (`/css/jktlug4.css`)
- The top banner with the jktlug logo and language switcher
...
section, "About jktlug", etc.) is inserted where `$body$` appears.
...
|-------|--------|----------------|
| `/css/jktlug4.css` | `docroot/css/jktlug4.css` | `match "docroot/**"` with `copyFileCompiler` |
| `/images/shirt200.png` | `docroot/images/shirt200.png` | Same |
| `/images/jktlug-uname.png` | `docroot/images/jktlug-uname.png` | Same |
...
Browser ──GET /──► Server
    ...
    (site structure)             (page content)
           ...
           ┌─────────────┴─────────────┐
           ▼                             ▼
     docroot/css/jktlug4.css       docroot/images/shirt200.png
     docroot/images/jktlug-uname.png
```
...
Browser ──GET /wiki/Meetings:2024:03──► Server
  ...
  │        │              │              │        │
  ▼        ▼              ▼              ▼        ▼
template  jktlug.MediaWiki  Pandoc  jktlug.WikiLink  template
